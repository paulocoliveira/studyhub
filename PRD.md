...
StudyHub is a full-stack web application built with Python and Django that serves as a personal learning content management system. It centralizes content scattered across multiple platforms — website links, YouTube videos, Instagram posts and profiles, PDF articles, and other resources — into a single, organized workspace. The system helps users save, categorize, track, and review their learning materials while providing AI-powered insights about their study habits.
...
- **FR-15:** Supported content types: article, video, podcast, social_media_post, social_media_profile, pdf, course, other
...
- **FR-20:** Content listing must support sorting by: date added, title, and status
- **FR-21:** Content listing must support text search across title and description
- **FR-22:** Users must be able to toggle between a card grid view and a list view on the content listing page. The selected view preference must persist across page reloads
- **FR-23:** In card view, each content item must display a thumbnail image: the Open Graph preview image extracted from the content URL, or a content-type-specific placeholder icon if no preview is available
...
- **FR-26:** Users must be able to create, edit, and delete categories
- **FR-27:** Each category must have a name and optional description
- **FR-28:** Categories are user-scoped (each user has their own categories)
- **FR-29:** Users must be able to list their categories with the number of contents in each

### 6.6 Tags

- **FR-30:** Users must be able to create and delete tags
- **FR-31:** Each tag must have a name
- **FR-32:** Tags are user-scoped
- **FR-33:** Tags can be assigned to content (many-to-many relationship)
- **FR-34:** Users must be able to list their tags with the number of contents in each

### 6.7 AI Insights

- **FR-35:** The system must offer AI-powered auto-suggestion of category for new content based on title and URL
- **FR-36:** The system must offer AI-powered auto-generation of description for new content
...
| **CSS** | TailwindCSS (via CDN or standalone CLI) |
| **AI** | Anthropic Claude API (via `anthropic` Python SDK) |
| **Auth** | Django native auth with custom User model |
| **Server** | Django development server (production server in final sprints) |
| **Link Preview** | `requests` + `beautifulsoup4` (Open Graph metadata extraction) |
...
| **Surface** | `bg-gray-800/50` | — | Elevated surfaces, modals |
...
| **Accent Primary** | `text-violet-500` | `#8B5CF6` | Primary actions, active states |
| **Accent Gradient From** | `from-violet-600` | `#7C3AED` | Gradient start |
| **Accent Gradient To** | `to-indigo-600` | `#4F46E5` | Gradient end |
| **Success** | `text-emerald-500` | `#10B981` | Completed status, success |
| **Warning** | `text-amber-500` | `#F59E0B` | In-progress status, alerts |
| **Danger** | `text-rose-500` | `#F43F5E` | Delete actions, errors |
| **Info** | `text-sky-500` | `#0EA5E9` | New status, info badges |
...
<!-- Subtle background gradient (cards, headers) -->
...
<!-- Border gradient effect (using pseudo-element or ring) -->
<div class="ring-1 ring-violet-500/20">
```
...
|---|---|
| **Font Family** | `font-sans` (system default: Inter, system-ui, sans-serif) |
...
<div class="bg-gradient-to-br from-gray-900 to-gray-900/50 
    ...
    <p class="text-sm text-gray-400">Total Contents</p>
    <p class="text-2xl font-bold text-gray-100 mt-1">128</p>
</div>
```
...
<!-- Other types follow same pattern with different colors -->
```
...
| ID | Story | Acceptance Criteria |
|---|---|---|
| US-07 | As a user, I want to add new content | Form with: title (required), URL (optional), content type (required), description (optional), category (optional), tags (optional), status (default: new). Content is saved and user is redirected to content list. |
| US-08 | As a user, I want to list all my content | Content list page with all saved items showing title, type badge, status badge, category, and date added. |
| US-09 | As a user, I want to filter my content list | Filters available for: status, content type, category. Filters update the list without full page reload if possible, or via form submission. |
| US-10 | As a user, I want to search my content | Text search across title and description. Search can be combined with filters. |
| US-11 | As a user, I want to sort my content list | Sorting options: newest first (default), oldest first, title A-Z, title Z-A. |
...
| US-13 | As a user, I want to edit my content | Edit form pre-filled with current values. On save, redirect to detail page. |
...
| US-15 | As a user, I want to quickly change content status | Status can be changed from the content list (inline) or from the detail page. |
| US-16 | As a user, I want to see my content in a card grid view with thumbnail images | Content list has a toggle for card/list view. Cards show: thumbnail (OG image or placeholder), title, type, status, date. Toggle preference persists via localStorage. |
| US-17 | As a user, I want to see link preview images on my content cards | When content has a URL, the system extracts the Open Graph image and displays it on the card. If no OG image is found, a content-type placeholder is shown. |
| US-18 | As a user, I want to upload a local file when saving content | Content form has an optional file upload field. Accepted formats: PDF, JPG, PNG, GIF, WebP, MP3, MP4, DOC, DOCX, TXT, MD. Max size: 10MB. Validation errors are shown on the form. |
...
| ID | Story | Acceptance Criteria |
|---|---|---|
| US-26 | As a user, I want AI to suggest a category for my content | Button on content form triggers AI call with title and URL. Suggested category is shown and can be accepted or rejected. |
...
| R2 | **AI API costs** | Medium | Medium | AI calls are user-triggered only (not automatic). Implement rate limiting per user. Cache responses where appropriate. |
| R3 | **AI API downtime** | Medium | Low | AI features are optional enhancements. App works fully without them. Show graceful error messages on failure. |
| R4 | **TailwindCSS CDN dependency** | Low | Low | Use TailwindCSS standalone CLI as a fallback. Bundle compiled CSS for production. |
...
| R6 | **Email-based auth edge cases** | Low | Low | Use Django's well-tested `AbstractBaseUser`. Add email normalization and validation. |
| R7 | **No automated tests initially** | Medium | Medium | Manual QA per sprint. Tests are planned for pre-final sprints. Structure code for testability from day one. |
| R8 | **Security vulnerabilities** | High | Low | Use Django's built-in security features (CSRF, XSS protection, password hashing). Keep Django updated. |
| R9 | **Malicious file uploads** | High | Medium | Enforce server-side validation of file extension and size (10MB limit). Use Django's `FileField` validators. Never serve uploaded files with executable content types. Restrict allowed extensions to a whitelist. Store uploads outside the static directory. |
| R10 | **Open Graph fetch abuse (SSRF)** | Medium | Low | Validate URLs before fetching: reject private/internal IPs, enforce HTTPS, set strict timeouts (5s), limit response size. Do not follow excessive redirects. |

---

*End of PRD — StudyHub v1.0*